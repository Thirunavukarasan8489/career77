    ...
    <nav className="bg-white border-b border-slate-200/80 sticky top-0 z-50 backdrop-blur-md bg-white/95">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-3.5 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {session && session.user && (session.user as any).role === "recruiter" ? (
          ...
          ) : (session && session.user && (session.user as any).role === "candidate") || candidate ? (
            <div className="flex items-center gap-3 text-xs sm:text-sm font-semibold">
              <Link
                href="/candidate"
              ...
              <Link
                href="/candidate"
                className="w-9 h-9 rounded-full bg-blue-100 text-blue-700 font-extrabold flex items-center justify-center border border-blue-200 hidden sm:flex"
              >
                {(session?.user?.name || candidate?.name)?.[0]?.toUpperCase()}
              </Link>
              <button
                onClick={() => {
                  if (session) signOut({ callbackUrl: "/" });
                  else logoutCandidate();
                }}
                className="border border-slate-200 text-slate-600 px-3.5 py-2 rounded-xl hover:bg-slate-50 transition-colors"
              >
                Logout
