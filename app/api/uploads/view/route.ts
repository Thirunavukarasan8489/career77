...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!fileRes.ok) {
      ...
      return new Response(
        `Failed to fetch file from storage: Status ${fileRes.status} (${fileRes.statusText}). Cloudinary response: ${errText}`,
        { status: 500 },
      );
    }
    ...
    return new Response(error.message || "Failed to view file", {
      status: 500,
    });
  }
}
